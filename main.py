...
from utils.metrics import get_metrices, get_report
...
class Trainer:
    ...
    def train(self, train_loader):
        ...
        for epoch in range(self.epoch):
            ...
            for _, train_batch in enumerate(train_loader):
                ...
                loss.backward()
                self.optimizer.step()
                logger.info(f"[train] epoch: {epoch+1} {global_step}/{total_step} loss: {loss.item()}")
                global_step += 1
    ...
    def test(self, test_loader):
        ...
        acc, precision, recall, f1 = get_metrices(seq_trues, seq_preds, "cls")
        report = get_report(seq_trues, seq_preds, "cls")
        ner_acc, ner_precision, ner_recall, ner_f1 = get_metrices(
            token_trues, token_preds, "ner"
        )
        ner_report = get_report(token_trues, token_preds, "ner")
        print(
            "意图识别：\naccuracy:{}\nprecision:{}\nrecall:{}\nf1:{}".format(
...
if __name__ == "__main__":
    ...
    if args.load_model and os.path.exists(args.load_dir):
